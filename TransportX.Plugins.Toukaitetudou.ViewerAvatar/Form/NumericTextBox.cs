...
namespace TransportX.Plugins.Toukaitetudou.ViewerAvatar.Form
{
    ...
    public abstract class NumericTextBoxBase<T> : TextBox where T : struct, IComparable<T>
    {
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            if (e.Key == Key.Enter)
            {
                if (ValueTryParse(Text, out var value))
                {
                    ...
                    be.UpdateSource();
                    Select(Text?.Length ?? 0, 0);     // カーソルを最後尾に
                }
                else
                {
                    SelectAll();
        ...
        class CustomValidationRule : ValidationRule
        {
            public override ValidationResult Validate(object value, CultureInfo cultureInfo)
            {
                ...
                if (value?.ToString()?.ToLower()?.Contains("e") ?? false)
                {
    ...
    #region class FloatTextBox
    /// <summary>
    /// <see cref="float"/>型の値の入力を受け付けるテキストボックス
    /// </summary>
    public class FloatTextBox : NumericTextBoxBase<float>
    {
        /// <summary>最大値と最小値の初期値を設定します</summary>
        protected override void SetDefaultMinMax()
        {
        ...
        /// <param name="value">変換された値</param>
        /// <returns>変換可能か</returns>
        protected override bool ValueTryParse(string text, out float value)
        {
            if (text.ToLower().Contains("e"))
            {  // 有効数字形を不可とする
                value = default(float);
